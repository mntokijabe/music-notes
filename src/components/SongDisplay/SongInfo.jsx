...
function SongInfo() {
  ...
  function addSong() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed)
        dispatch({
          type: "ADD_TO_REPERTOIRE",
          payload: {
            ensemble_id: activeEnsemble[0].ensemble_id,
  ...
  return (
    <Container display="flex" sx={{ display: "flex", flexDirection: "row" }}>
      <Box sx={{ width: "350px" }}>
        <table>
          <tbody>
            ...
            <tr style={{ height: "30px" }}>
              {" "}
              <td style={{ width: "100px" }}>Composer:</td>
              <td>{songInfo.composer}</td>
            </tr>
            <tr style={{ height: "30px" }}>
              {" "}
              <td>Arranged by:</td>
              <td>{songInfo.arranged_by}</td>
            </tr>
            <tr style={{ height: "30px" }}>
              {" "}
              <td>Voicing:</td>
              <td>{songInfo.voicing}</td>
            </tr>
            <tr style={{ height: "30px" }}>
              {" "}
              <td>Genre:</td>
            ...
            <tr style={{ height: "30px" }}>
              {" "}
              <td>Publisher:</td>
              <td>{songInfo.publisher}</td>
            </tr>
            <tr style={{ height: "30px" }}>
              {" "}
              <td>Copyright:</td>
              <td>{songInfo.copyright_year}</td>
            </tr>
            <tr style={{ height: "50px" }}>
              {" "}
              <td>Copies on Hand:</td>
              <td>{songInfo.quantity}</td>
            </tr>
          </tbody>
        </table>
        {user.admin === true && (
          <Box sx={{ display: "flex", gap: 1, marginTop: "40px" }}>
            ...
            <Button
              component={Link}
              to={`/notes/${songInfo.id}`}
              variant="contained"
              sx={{ width: "100px", textAlign: "center" }}
            >
              View or Post Performance Notes
            </Button>
            <Button
        ...
        {user.admin === false && (
          <Button
            ...
            to="/user"
            variant="contained"
            sx={{ width: "150px", textAlign: "center", mt: "30px", ml: "80px" }}
          >
      ...
      <Paper
        sx={{ marginLeft: "30px", display: "flex", justifyItems: "center" }}
      >
