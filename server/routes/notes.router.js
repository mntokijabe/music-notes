...
router.post("/", rejectNonAdmin, rejectUnauthenticated, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const performanceId = await connection.query(
        queryPerformanceText,
        queryPerformanceValues
      );
      const plainId = performanceId.rows[0].id;
      const queryPerformanceSongValues = [
        performanceId.rows[0].id,
        performance.songId,
        performance.note,
        performance.ensemble,
      ];
      await connection.query(
        queryPerformanceSongText,
        queryPerformanceSongValues
      );
      res.sendStatus(200);
    } finally {
      connection.release();
